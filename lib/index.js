...
async function applyNewSource(realDir, root, preserveFiles = []) {
  ...
  try {
    ...
    const entryExistsNow = () => {
      ...
      return entry === '' || existsSync(join(realDir, entry))
    }
    let installErr = ''
...
class PluginUpdatesGateway extends TypertRemoteService {
  ...
  constructor(ctx) {
    ...
    const methods = ['status', 'checkNow', 'update', 'updateAll', 'uninstall', 'setEnabled', 'updateAssetPlugin']
  ...
  status() {
    ...
    if (cached) {
      return { ...cached, checking: this.checking !== null, repair: this.lastRepair }
    }
    return {
      checkedAt: null,
      checking: true,
  ...
  async updateAllLocked() {
    ...
    const specs = targets.map((entry) => `${validName(entry.name)}@latest`)
    ...
    if (run.code !== 0 && !anyChanged) {
      return { ok: false, error: cliFailure(run, 'updateAll'), results: [] }
    }
    reconcileBundles()
    this.refresh().catch(() => {})
    return {
