...
const ZAT_ENGINE_PKG = 'zat-dsh-engine'
...
/** zat-dsh-engine（插件市场引擎）检测：已装 → 状态；未装 → 安装提示。 */
function zatEngineStatus() {
  const manifest = manifestOf()
  const bundles = Array.isArray(manifest.dsh?.profile?.bundles) ? manifest.dsh.profile.bundles : []
  const inBundles = bundles.includes(ZAT_ENGINE_PKG)
  const pkgPath = path.join(profileDir(), 'node_modules', ZAT_ENGINE_PKG, 'package.json')
  const pkg = existsSync(pkgPath) ? readJson(pkgPath) : null
  const srcPkgPath = path.join(homeDir(), 'plugin-src', ZAT_ENGINE_PKG, 'package.json')
  const srcPkg = existsSync(srcPkgPath) ? readJson(srcPkgPath) : null
  const version = pkg?.version ?? srcPkg?.version ?? null
  return {
    installed: inBundles || pkg !== null,
    ...
    inBundles,
    nodeModules: pkg !== null,
    installHint: `dsh plugin --profile ${PROFILE_NAME} add ${ZAT_ENGINE_PKG}`,
