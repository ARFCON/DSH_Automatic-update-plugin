/**
 * dsh-hub — host half（插件中枢：更新引擎 + 自身更新检查）。
 *
 ...
 *  2. 自身更新检查：读 GitHub 仓库 package.json 的 version 对比本地版本
 *     （raw.githubusercontent + jsDelivr CDN 双源，规避 GitHub API 限流 403）
 *
 * Remote 服务 `dshHub` 暴露给客户端设置页（9 个方法）：
 *   - status():            总览（插件列表 / Desktop / 客户端插件 / 自更新）
 *   - checkNow():          立即全量检查插件更新
 ...
 *
 * 每次宿主进程启动自动执行：启动自检修复（~0.6s）→ 插件更新检查 + 自身更新检查（~1.5s）。
 *
...
function runCli(command, args, timeoutMs, options = {}) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      try { child.kill() } catch { /* 有意吞掉：尽力而为的清理/读取，失败不影响主流程 */ }
  ...
  const workers = Array.from({ length: Math.min(REGISTRY_BATCH_PARALLEL, Math.max(1, chunks.length)) }, async () => {
    while (true) {
      ...
      try {
        ...
        try { rmSync(dir, { recursive: true, force: true }) } catch { /* 有意吞掉：尽力而为的清理/读取，失败不影响主流程 */ }
      }
    }
  })
...
const KNOWN_CLIENT_PLUGIN_REPOS = {
  ...
  'zat-dsh-engine': { owner: 'mishibeikejie', repo: 'zat-dsh-engine' },
}
...
/** 把 collectGithubTags 的结果整理成客户端要展示的 GitHub 信息块（保持与旧格式一致）。 */
function entryGithubInfo(github) {
  const repo = github?.repo ?? null
  if (repo === null) return null
  return {
    owner: repo.owner,
    repo: repo.repo,
    latestTag: github?.value?.tag ?? null,
    htmlUrl: `https://github.com/${repo.owner}/${repo.repo}`,
    updateUrl: `https://github.com/${repo.owner}/${repo.repo}/${(github?.value?.hasRelease ?? false) ? 'releases/latest' : 'tags'}`,
  }
}

/** 单个依赖条目 → 客户端更新列表行（registry/local/git 三种来源）。 */
function entryFromDep(dep, npmMap, githubMap) {
  if (dep.source === 'registry') {
    const latest = npmMap.get(dep.name) ?? null
    return { ...dep, latest, updateable: hasNewerVersion(latest, dep.current) }
  }
  if (dep.source === 'local') {
...
/** 装配状态字段（entryId / enabled / isDeveloper）。 */
function entryEnabled(entry, insertMap, disableIds) {
...
async function collectCheck() {
  ...
  const entries = deps
    .map((dep) => entryFromDep(dep, npmMap, githubMap))
    .map((entry) => entryEnabled(entry, insertMap, disableIds))
...
function rollbackPlugin(realDir, backupDir) {
  ...
  if (existsSync(nmDir) && !existsSync(nmStore)) {
    try { renameSync(nmDir, nmStore) } catch { /* 有意吞掉：尽力而为的清理/读取，失败不影响主流程 */ }
  }
...
async function applyNewSource(realDir, root, preserveFiles = []) {
  ...
  try {
    ...
    let nmMoved = false
    if (existsSync(nmDir)) {
      try { renameSync(nmDir, nmStore); nmMoved = true } catch { /* 有意吞掉：尽力而为的清理/读取，失败不影响主流程 */ }
    }
...
function ensureSelfActivation(deps, path = patchPath()) {
  if (!deps.some((entry) => entry.name === 'dsh-hub')) return []
  const text = existsSync(path) ? readFileSync(path, 'utf8') : ''
  if (/name:\s*['"]dsh-hub['"]/.test(text)) return []
  try {
    writeTextSafe(path, text.replace(/\s+$/, '') + "\n- insert:\n    - id: dsh-hub\n      name: 'dsh-hub'\n      config: {}\n")
    return ['补回了 dsh-hub 自身的激活行']
  } catch {
    return []
  }
}

/** 按插件名移除 cordis.patch.yml 里的 insert 块（同族去重时停用 loser 用）。 */
function removeInsertBlockByName(name) {
  const path = patchPath()
  if (!existsSync(path)) return false
  const text = readFileSync(path, 'utf8')
  const parts = text.split(/(?=^- insert:)/m)
  let removed = false
  const kept = parts.filter((part) => {
    if (!part.startsWith('- insert:')) return true
    const m = part.match(/^\s*name:\s*['"]([^'"]+)['"]/m)
    if (m && m[1] === name) {
      removed = true
      return false
    }
  ...
  if (removed) writeTextSafe(path, kept.join(''))
  return removed
}

/**
 * dsh-hub / dsh-hotplug-hub 同族去重：两个同族插件同时被装配时，只保留
 * 「本地版优先，其次版本最新」的 winner，loser 从 dsh.profile.bundles 与
 * cordis.patch.yml 激活行中移除（不删依赖、不跑 pnpm，重启后只加载 winner）。
 */
function reconcileHubDuplicates() {
  const actions = []
  const manifest = readJson(manifestPath())
  if (!manifest) return actions
  const dependencies = manifest.dependencies ?? {}
  const family = ['dsh-hub', 'dsh-hotplug-hub']
  const present = family.filter((name) => Object.prototype.hasOwnProperty.call(dependencies, name))
  if (present.length < 2) return actions
  const entries = present.map((name) => {
    const pkg = readJson(join(packageDir(name), 'package.json')) ?? {}
    return {
      name,
      version: typeof pkg.version === 'string' && pkg.version ? pkg.version : '0.0.0',
      local: classifySpec(String(dependencies[name] ?? '')) === 'local',
    }
  })
  // 规则：本地版优先；同为本地/远端时取版本号最高者。
  entries.sort((a, b) => {
  ...
  for (const loser of losers) {
    const tag = (entry) => `${entry.name} ${entry.version}${entry.local ? '(本地版)' : ''}`
    if (bundles.includes(loser.name)) {
      manifest.dsh.profile.bundles = bundles.filter((bundle) => bundle !== loser.name)
      actions.push(`${tag(winner)}，从 bundles 移除 ${tag(loser)}`)
    } else {
      actions.push(`${tag(winner)}；${loser.name} 未在 bundles 中，无需移除`)
    }
    if (removeInsertBlockByName(loser.name)) {
      actions.push(`${loser.name} 的激活行`)
    }
  }
  // 保底：winner 必须留在 bundles（否则把两个都移除后本插件反而没了）。
  if (winner.name === 'dsh-hub' && !manifest.dsh.profile.bundles.includes('dsh-hub')) {
    manifest.dsh.profile.bundles.push('dsh-hub')
    actions.push('dsh-hub 同族去重：确保 dsh-hub 保留在 bundles 中')
  }
  if (losers.length > 0) {
...
function repairAll() {
  ...
  actions.push(...reconcileHubDuplicates())
...
class HubGateway extends TypertRemoteService {
  ...
  constructor(ctx) {
    ...
    const methods = ['status', 'checkNow', 'update', 'updateAll', 'uninstall', 'setEnabled', 'updateAssetPlugin', 'checkUpdate', 'updateSelf', 'repairNow']
  ...
  runStartupRepair() {
    ...
    const run = Promise.resolve().then(() => {
      ...
      if (this.lastRepair.actions.length > 0) this.refresh().catch(() => { /* 有意吞掉：后台刷新失败不打断前台 */ })
...
export function apply(ctx) {
  // 注册 Remote 网关（构造时启动后台任务：自检修复 + 插件更新检查 + 自更新检查）。
  new HubGateway(ctx)
}
